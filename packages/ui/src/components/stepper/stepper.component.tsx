...
const Step = ({ text, details, iconName }: Step) => {
  return (
    <li className="mb-10 ms-6">
      ...
      <h3 className="font-medium leading-tight">{text}</h3>
      {details && <p className="text-sm">{details}</p>}
    </li>
  );
};

const CurrentStep = ({ text, details, iconName, withLoader }: Step) => {
  return (
    <li className="mb-10 ms-6">
      {!withLoader ? (
        ...
        <span className="absolute flex items-center justify-center w-8 h-8 bg-blue-100 rounded-full -start-4 ring-4 ring-white dark:ring-blue-900 dark:bg-blue-700">
          <svg
            aria-hidden="true"
            className="text-gray-200 animate-spin dark:text-gray-400 fill-white"
            viewBox="0 0 100 101"
            fill="none"
...
const SuccessStep = ({ text, details, iconName }: Step) => {
  return (
    <li className="mb-10 ms-6">
